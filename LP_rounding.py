...
def create_model_lp(input_params,map_args,cost):
  ...
  m.addConstr(quicksum(X[i]+Y[i] for i in range(0, len(input_params['attribute_set'])))<=cost)  # int(budget)
  ...
  if m.status == GRB.Status.OPTIMAL:
    ...
    for k in range(0, int(map_args.iterations)):
      ...
      if budget_check <= (2 * cost) and no_of_hits_z1 >= (int(map_args.M1) / 8.0) and no_of_hits_z2 >= (int(map_args.M2) / 8.0):
    ...
    return (count_final_z1+count_final_z2)/len(input_params['dataset'])
  else:
    print("No solution.")
    return 0
...
if __name__ == '__main__':
  ...
  plt.ylabel("% Coverage")
  plt.savefig(map_args.input_file[:-4] + '_lp_results_2.png')
